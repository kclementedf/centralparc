...
centrale_statut_df = pd.read_parquet(os.path.join(data_folder_path, "centrale_statut.parquet"))
...
productible_reel_mensuel_df["PERIODE_MENSUELLE"] = pd.to_datetime(productible_reel_mensuel_df["PERIODE_MENSUELLE"])
...
contrat_interlocuteur = pd.read_excel(os.path.join(data_folder_path, "contrat_interlocuteur.xlsx"))
...
centrale_evenement_df = pd.read_excel(os.path.join(data_folder_path, "centrale_evenement.xlsx"))
projet_evenement_df = pd.read_excel(os.path.join(data_folder_path, "projet_evenement.xlsx"))
...
cross = centrale_df[['Plant ID', "RAISON_SOCIALE"]].merge(sap_project_company_df[['Company Name']], how='cross')
cross['match_acc'] = cross.apply(lambda x: fuzz.ratio(x["RAISON_SOCIALE"], x["Company Name"]), axis=1)
centrale_df = centrale_df.merge(
    cross.sort_values(["Plant ID", "match_acc"], ascending=[True, False]).drop_duplicates(["Plant ID"])[["Plant ID", "Company Name"]],
    how='left'
    )


#%%
centrale_df["antenne"] = centrale_df.apply(lambda x: x["ID_ANTENNE_PDL"] if x["ID_ANTENNE_PDL"] is not None else x["ID_ANTENNE"], axis=1)
centrale_df["antenne"] = centrale_df["antenne"].round(0)
centrale_df["antenne"] = centrale_df["antenne"].replace(antenne_to_bluepoint).fillna("")

centrale_df["ID_POLE"] = centrale_df["ID_POLE"].round(0).replace(pole_to_bluepoint).fillna("")
...
contrat_df["Plant Identifier"] = contrat_df["ID_CONTRAT"].replace(contrat_plant)

contrat_df = contrat_df.merge(
    #                                                       142 -> Prestataire
    contrat_interlocuteur[contrat_interlocuteur["ID_TYPE_CONTACT"]==142][["ID_CONTRAT", "ID_INTERLOCUTEUR"]],
    left_on=["ID_CONTRAT"],
    right_on=["ID_CONTRAT"],
    how="left"
).merge(
...
import_contract_df = import_contract_df[(import_contract_df["ID_TYPE_CONTRAT"] != 137) & (~import_contract_df["ID_TYPE_CONTRAT"].isna())]
import_contract_df["ID_TYPE_CONTRAT"] = import_contract_df["ID_TYPE_CONTRAT"].replace(contrat_type_to_bluepoint)
import_contract_df = import_contract_df.rename(columns=contrat_to_bluepoint)[list(contrat_to_bluepoint.values())]
import_contract_df = pd.concat([
    import_contract_df[import_contract_df["Contract Type"].str.contains("Vente")==False],

    import_contract_df[import_contract_df["Contract Type"].str.contains("Vente")==True].rename(columns={"Vendor": "Client", "Company Name": "Vendor"})
])
...
import_milestone_df["ID_EVENEMENT"] = import_milestone_df["ID_EVENEMENT"].replace(milestone_to_bluepoint)

import_milestone_df = import_milestone_df[list(milestone_to_bluepoint.keys())].rename(columns=milestone_to_bluepoint)
...
milestone_df["Date"] = pd.to_datetime(milestone_df["Date"], format='mixed', errors='coerce')
milestone_df["Actual"] = "Yes"
milestone_df["Milestone Type"] = milestone_df["Milestone Type"].replace(milestone_type_to_bluepoint)
...
milestone_df.to_excel(os.path.join(export_path, "5.milestone.xlsx"), index=False)
#%% WSP
event_to_bluepoint = {
"ID": "Event ID",

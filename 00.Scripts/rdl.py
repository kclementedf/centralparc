...
df = rdl_api.business_metadata(StaticDataView.PROJECT, filters="iso_country_code", value="FR", asd_df=True)
df = df[df["project_type"]=="Wind Onshore"]
if limit_scope_to:
    project_list = [project for project in df if project["project_code"] in limit_scope_to]
